		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'radio_img':
					...
					echo '<div class="cmb_radio_img">';
					foreach ($field['options'] as $option) {
						echo '<div class="cmb_radio_img_option">' . $option['name'], '<input type="radio" name="', $field['id'], '" value="', $option['value'], '"', $meta == $option['value'] ? ' checked="checked"' : '', ' />', '</div>';
					}
					echo '</div>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				...
				case 'file':
					...
					if ( 'url' == $field['allow'] || ( is_array( $field['allow'] ) && in_array( 'url', $field['allow'] ) ) )
						$input_type_url="text";
					echo '<input class="upload_file" type="' . $input_type_url . '" size="40" id="', $field['id'], '" name="', $field['id'], '" value="', $meta, '" />';
					echo '<input class="upload_button button" type="button" value="Upload File" />';
...
function cmb_styles_inline() { 
	...
	?>	
	<style type="text/css">
		table.cmb_metabox { margin: 0; }
		table.cmb_metabox input[type="text"] { padding: 4px 3px; }
		table.cmb_metabox td, table.cmb_metabox th { border-bottom: 1px solid #E9E9E9; padding: 15px 10px 10px 1px; }
		table.cmb_metabox th { font-weight: bold; }
		table.cmb_metabox th label { margin-top: 2px; display: block; }
		p.cmb_metabox_description { margin: 2px 0 10px !important; }
		.cmb_metabox_description { color: #666; font-style: italic; line-height: 1.5; }
		.cmb_metabox_description em { display: block; font-size: 11px; color: #999; margin-bottom: 12px; }
		...
		.cmb_metabox_title { margin: 0 0 5px 0; padding: 5px 0 0; font: italic 24px/35px Georgia,"Times New Roman","Bitstream Charter",Times,serif; }
		.cmb_radio_inline { padding: 4px 0 0; }
		.cmb_radio_inline_option { display: inline; padding-right: 18px; }
		.cmb_radio_img { overflow: hidden; padding: 10px 0; }
		.cmb_radio_img_option { float: left; margin-right: 20px; text-align: center; }
		.cmb_radio_img_option div { font-size: 10px; line-height: 1.5; padding: 2px 0 5px; }
		...
		table.cmb_metabox .cmb_upload_status { margin: 10px 0 0; }
		table.cmb_metabox .cmb_upload_status .img_status { position: relative; }
		table.cmb_metabox .cmb_upload_status .img_status img { border: 1px solid #dfdfdf; background: #fafafa; max-width: 350px; padding: 5px; border-radius: 2px; }
