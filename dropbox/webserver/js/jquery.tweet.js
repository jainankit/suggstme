  ...
  $.fn.tweet = function(o){
    ...
    $.fn.extend({
	  ...
	  username: function() {
        var returning = [];
        this.each(function() {
          returning.push(this.replace(/(&lt;)+[3]/gi, ' <a href="http://search.twitter.com/search?q=&tag=$1&lang=all&from='+s.username.join("%2BOR%2B")+'">#$1</a>'));
        });
        return $(returning);
      }
    });
    ...
    return this.each(function(){
	  ...
	  var username = $('<p class="username">'+s.username+'</p>');
      ...
      $.getJSON(build_url(), function(data){
        ...
        $.each((data.results || data), function(i,item){
          ...
          var text = '<span class="tweet_text">' +$([item.text]).linkUrl().linkUser().linkHash().makeHeart().capAwesome().capEpic()[0]+ "</span><p class=\"profileLink\"><a href=\"http://twitter.com/"+s.username+"\">"+s.username+"</a></p>";
